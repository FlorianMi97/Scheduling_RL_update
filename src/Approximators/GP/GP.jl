...
Base.@kwdef mutable struct GP <: AbstractApproximator
    ...
    # sampling
    antithetic::Bool = false # if true use antithetic sampling
...
function GP(prim::Array{Function}, feat::Array{Symbol}, mD::Int, pa::Dict{Symbol, Int}, envs::Vector{Env}, objective::Vector{Any};
            rng = Random.default_rng(), weight_mean=1.0, weight_max=0.0, kwargs...)
    ...
    reward = [weight_mean, weight_max]

    GP(;primitives=prim, features=feat, maxDepth=mD, primArity = pa, envs = envs, objective = objective, reward = reward,
...
"""
    ...
    (optional) kwargs:  stationary_samples::Bool = true # if false change samples each x generations
                        ...
                        .....
"""
function createGP(envs; objective = [], features = Set(), primitives = Set(), maxdepth = 5,simplerules =false, kwargs...)
    # TODO use settings to customize GP, store them somewhere to recreate agent! / save the agent.
                ...
                if !isempty(features) stringFeatures = [s for s in stringFeatures if s in features] end
                ...
                if isempty(primitives)
                    ...
                    PRIMITIVES = [possibleprimitives[p] for p in primitives]
                end
                ...
                if "antithetic" in keys(kwargs)
                    if kwargs["antithetic"] == true
                        @assert "samples_generation" not in keys(kwargs) || kwargs["samples_generation"] % 2 == 0 "antithetic sampling only works with an even number of samples per generation"
                    end
                end
                GP(PRIMITIVES, FEATURES, maxdepth, arity, envs, objective; kwargs...)
...
function initialpop(gp::GP)
    ...
    setsamples!.(gp.envs, gp.rng; nSamples = n, gp.antithetic) #TODO number of samples specifyable, used RNG either for seeds or in uncertainity function
    population = []
...
function drawTree(tree_expr::Expr, stage::Int, path::String, reduced = false, feature = nothing)
    plot(tree_expr, method=:tree, nodeshape=:rect, curves=false, fontsize=10, nodecolor="white", nodesize=0, shorten=0.15, size=(1500,1000))
    if reduced == false
        savefig(plot!(), path)
        else
        savefig(plot!(), "$path\\treePlot$(stage)reducedBy$(feature).svg")
    end
end
...
function next_gen!(gp::GP, pop, ngen)
    ...
    if !gp.stationary_samples
        if ngen != 1 && (ngen-1) % gp.change_every == 0
            if gp.sample_cycling
                gp.sample_tens_digit = ((ngen-1) ÷ gp.change_every) % (gp.steps_in_cycle)
            else
                setsamples!.(gp.envs, gp.rng, nSamples = gp.samples_generation, gp.antithetic)
            end
        end
    end
...
function new_pop(next_gen, gp::GP, pop, ngen)
        ...
        if ngen != 1 && (ngen-1) % gp.change_every == 0
            for ind in pop
                ...
                for e in eachindex(gp.envs)
                    tmpF, individual_fitness, _ = evalfitness([get_expression(ind[1][k],gp) for k in 1:gp.stages], gp.envs[e],
                        gp.objective, gp.samples_generation, gp.rng, gp.sample_tens_digit) #  ;type = "greedy") # -> sum(fitness),fitness, metrics
                    fitness += (gp.reward[1] * (tmpF / gp.samples_generation) + gp.reward[2] * maximum(individual_fitness))
                    trueobj += (tmpF / gp.samples_generation)
                end
                fitness /= length(gp.envs)
                trueobj /= length(gp.envs)
    ...
    if !gp.markerdensity
        ...
        # increase age of old individuals by 1
        for i in pop
            i[3] += 1
        end
        ...
        # always add best inidvidual to new pop
        tmpbest = newPop[argmin([i[2] for i in newPop])]
        ...
        # in the rare case that the best individual was removed add it back
        if tmpbest ∉ newPop
            append!(newPop, tmpbest)
        end
...
function pareto_tournament_removal!(pop, list, gp::GP)
    if length(list) == 2
        if list[1][2] == list[2][2] && list[1][6] == list[2][6]
            remove = tiebreaker(list[1], list[2], gp)
            index = findfirst(x -> x == remove, pop)
            deleteat!(pop, index)
        elseif (list[1][2] >= list[2][2] && list[1][6] >= list[2][6]) 
...
function detF(indi, age, gp::GP)
    ...
    trueobjective = 0
    for e in eachindex(gp.envs)
        tmpF, individual_fitness, _ = evalfitness([get_expression(indi[k],gp) for k in 1:gp.stages], gp.envs[e],
            gp.objective, gp.samples_generation, gp.rng, gp.sample_tens_digit) #  ;type = "greedy") # -> sum(fitness),fitness, metrics
        fitness += (gp.reward[1] * (tmpF / gp.samples_generation) + gp.reward[2] * maximum(individual_fitness))
        trueobjective += (tmpF / gp.samples_generation)
    end
    fitness /= length(gp.envs)
    trueobjective /= length(gp.envs)

    if gp.markerdensity
        Ftuple = tuple([indi, fitness, age, trueobjective, get_marker(indi,gp)[1], 0]) # nothing as placeholder for marker; 0 as placeholder for density
    else
        Ftuple = tuple([indi, fitness, age, trueobjective])
    end
    return Ftuple
end
...
function assign_marker_density!(pop)
    ...
    for (i,individum) in enumerate(pop)
        # get marker
        marker = individum[5]  # TODO if multiple rules for each stage change this
...
function sortallsamples!(pop, gp)
    ...
    for ind in pop
        ...
        for e in eachindex(gp.envs)
            ...
            fitness += (gp.reward[1] * (tmpF / nrEvals) + gp.reward[2] * maximum(individual_fitness))
            trueobjective += (tmpF / nrEvals)
        end
        fitness /= length(gp.envs)
        trueobjective /= length(gp.envs)
        ind[2] = fitness
...
function validate(pop, gp, validationenvs; keep_pop=true, selection="reward")
    sorted_pop = keep_pop ? deepcopy(pop) : pop
    for ind in sorted_pop
        ind[2] = 0
        ind[4] = 0
        fitness = 0
        trueobjective = 0
        nrEvals = max(length(validationenvs[1].samples),1)
        for env in validationenvs
            tmpF, individual_fitness, _ = evalfitness([get_expression(ind[1][k], gp) for k in 1:gp.stages], env,
                gp.objective, nrEvals, gp.rng, 0) #  ;type = "greedy") # -> sum(fitness),fitness, metrics
            fitness += (gp.reward[1] * (tmpF / nrEvals) + gp.reward[2] * maximum(individual_fitness))
            trueobjective += (tmpF / nrEvals)
        end
        fitness /= length(validationenvs)
        trueobjective /= length(validationenvs)
        ind[2] = fitness
        ind[4] = trueobjective
    end
    if selection == "reward"
        sort!(sorted_pop, by=i->i[2])
    elseif selection == "objective"
        sort!(sorted_pop, by=i->i[4])
    end
    return sorted_pop
end
...
function analyze_tree(expr)
    #@warn "Only works for add and sub operations and apply simplify_expression() and  reduceTreeNumericals() fist "
 ...
 function Covert_weights_from_GP(featureToPrimitivesMapping, FeatureCounter)

    predefined_features = ["PT", "DD", "RO", "RW", "ON", "JS", "ST", "NI", "NW", "SLA", "EW", "JWM", "CW", "CJW", "CT", "TT", "BSA", "DBA", "RF"]
    
    Initial_best_weights = Vector{Float32}()
    Initial_best_weights_normalized = Vector{Float32}()
    weightsigns = Vector{Int}()  # Vector to store the sign of each weight

    for predefined_feature in predefined_features
        symbol_feature = Symbol(predefined_feature)
        if haskey(FeatureCounter, symbol_feature)
            weight = FeatureCounter[symbol_feature]
            sign = haskey(featureToPrimitivesMapping, symbol_feature) && featureToPrimitivesMapping[symbol_feature] == :sub ? -1 : 1
            push!(Initial_best_weights, Float32(weight * sign))
            push!(Initial_best_weights_normalized, Float32(abs(weight)))  # Use abs to ensure positivity
            push!(weightsigns, sign)  # Store the sign
        end
    end

    return Initial_best_weights, Initial_best_weights_normalized, weightsigns
end