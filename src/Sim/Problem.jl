...
function Problem(layout, instance, file; id = 1, randomorderbook = false, nOrders = 55, rng = Random.default_rng(), kwargs...)
    ...
    if layout == "Flow_shops"
        ...
        if randomorderbook == true
            if Seed !== nothing
                Random.seed!(Seed)
            end
            orderdistribution = data["arrival_prob"]
            ordersDict = sampleorders(orderdistribution, nOrders, rng)
        else
...
function sampleuncertainties(problem, nSamples, rng::AbstractRNG) 
    sample_dict = Dict(s=>Dict("processing_time" => Dict(j => Dict(o => Dict() for o in problem.ops_per_prod[problem.orders[j].type]) 
                        ...
                        , "setup_time" => Dict{Any, Any}(m => Dict{Any, Any}(pr => Dict() for pr in vcat(problem.prods,[nothing]))
        ...
        for j in keys(problem.orders)
            ...
            for o in job.ops
                for m in job.eligmachines[o]
                    ...
                    if problem.setupstatejob[m][nothing][j]["dist"] == "None"
                        sample_dict[s]["setup_time"][m][nothing][j] = 0.0
                    elseif problem.setupstatejob[m][nothing][j]["dist"] == "Deterministic"
                        sample_dict[s]["setup_time"][m][nothing][j] = problem.setupstatejob[m][pr][j]["mean"]
                    else
                        sample_dict[s]["setup_time"][m][nothing][j] = problem.setupstatejob[m][pr][j]["min"] + rand(rng,problem.setupstatejob[m][pr][j]["dist"])
                    end   
                end
            end
        end
...
function sampleuncertainitiesantithetic(problem, nSamples, rng::AbstractRNG)
    @assert(nSamples % 2 == 0, "nSamples has to be even")
    sample_dict = Dict(s=>Dict("processing_time" => Dict(j => Dict(o => Dict() for o in problem.ops_per_prod[problem.orders[j].type]) 
                                                    for j in keys(problem.orders))
                        , "setup_time" => Dict{Any, Any}(m => Dict{Any, Any}(pr => Dict() for pr in vcat(problem.prods,[nothing]))
                                                    for m in problem.machines) 
                        ) for s in 1:nSamples)
    for s in 1:2:nSamples
        for j in keys(problem.orders)
            job = problem.orders[j]
            # prod = j.type
            for o in job.ops
                for m in job.eligmachines[o]
                    ...
                    if job.processingtimes[o][m]["dist"] == "None"
                        sample_dict[s]["processing_time"][j][o][m] = 0.0
                        sample_dist[s+1]["processing_time"][j][o][m] = 0.0
                    elseif job.processingtimes[o][m]["dist"] == "Deterministic"
                        sample_dict[s]["processing_time"][j][o][m] = job.processingtimes[o][m]["mean"]
                        sample_dict[s+1]["processing_time"][j][o][m] = job.processingtimes[o][m]["mean"]
                    else
                        x,y = antithetic(job.processingtimes[o][m]["dist"], rng)
                        sample_dict[s]["processing_time"][j][o][m] = job.processingtimes[o][m]["min"] + x
                        sample_dict[s+1]["processing_time"][j][o][m] = job.processingtimes[o][m]["min"] + y
                    end
                    #sample setup time (dependent on predecessor)
                    for pr in problem.prods #setdiff(problem.prods, [prod])
                        if problem.setupstatejob[m][pr][j]["dist"] == "None"
                            sample_dict[s]["setup_time"][m][pr][j] = 0.0
                            sample_dict[s+1]["setup_time"][m][pr][j] = 0.0
                        elseif problem.setupstatejob[m][pr][j]["dist"] == "Deterministic"
                            sample_dict[s]["setup_time"][m][pr][j] = problem.setupstatejob[m][pr][j]["mean"]
                            sample_dict[s+1]["setup_time"][m][pr][j] = problem.setupstatejob[m][pr][j]["mean"]
                        else
                            x,y = antithetic(problem.setupstatejob[m][pr][j]["dist"], rng)
                            sample_dict[s]["setup_time"][m][pr][j] = problem.setupstatejob[m][pr][j]["min"] + x
                            sample_dict[s+1]["setup_time"][m][pr][j] = problem.setupstatejob[m][pr][j]["min"] + y
                        end
                    end 
                    if problem.setupstatejob[m][nothing][j]["dist"] == "None"
                        sample_dict[s]["setup_time"][m][nothing][j] = 0.0
                        sample_dict[s+1]["setup_time"][m][nothing][j] = 0.0
                    elseif problem.setupstatejob[m][nothing][j]["dist"] == "Deterministic"
                        sample_dict[s]["setup_time"][m][nothing][j] = problem.setupstatejob[m][pr][j]["mean"]
                        sample_dict[s+1]["setup_time"][m][nothing][j] = problem.setupstatejob[m][pr][j]["mean"]
                    else
                        x,y = antithetic(problem.setupstatejob[m][nothing][j]["dist"], rng)
                        sample_dict[s]["setup_time"][m][nothing][j] = problem.setupstatejob[m][pr][j]["min"] + x
                        sample_dict[s+1]["setup_time"][m][nothing][j] = problem.setupstatejob[m][pr][j]["min"] + y
                    end
                end
            end
        end
...
function antithetic(N,rng::AbstractRNG)
    u1_x = rand(rng)
    u1_y = 1-u1_x
    x = quantile(N,u1_x)
    y = quantile(N,u1_y)
...
function sampleorders(distro, nOrders, rng::AbstractRNG)
    orderbook = Dict()
    digits = length(string(nOrders))
    products = collect(keys(distro))
    weights = collect(values(distro))
    for i in 1:nOrders
        orderbook["ORD_$(lpad(i,digits,"0"))"] = Dict("due_date" => nothing, "product" => sample(rng, products, Weights(weights)))
    end
    return orderbook
end