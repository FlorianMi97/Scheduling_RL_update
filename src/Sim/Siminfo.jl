...
function initsiminfo(p::Problem, am)
    ...
    if "dds" ∈ am
        ...
        id_type_machine = nothing
    elseif "ois" ∈ am
        id_type_machine = Dict(type => Dict(machine => Dict("scheduling" => Inf, "waiting" => Inf) for machine in p.machines) for type in keys(p.mapindexprods))
        for j ∈ 1:length(p.orders)
            o = p.orders[j]
            for m in o.eligmachines[o.ops[1]]
                if parse(Int64, o.id[5:end]) < id_type_machine[o.type][m]["scheduling"]
                    id_type_machine[o.type][m]["scheduling"] = parse(Int64, o.id[5:end])
                    id_type_machine[o.type][m]["waiting"] = parse(Int64, o.id[5:end])
                end
            end
        end
    ...
    Siminfo(    nextevent = ne, pointercurrentop = Dict(j => 1 for j in keys(p.orders)), pointersample = nothing,
                ...
                numtardyjobs = 0, numsetups = 0, mappingjobs = mj , mappingmachines = mm, edd_type_machine = edd_type_machine, id_type_machine = id_type_machine
            )
end
...
function update_id!(env::AbstractEnv)
    for m ∈ env.problem.machines
        IDS = Dict(prod => Inf for prod ∈ keys(env.problem.mapindexprods))
        IDW = Dict(prod => Inf for prod ∈ keys(env.problem.mapindexprods))
        for j ∈ 1:length(env.problem.orders)
            if env.siminfo.pointercurrentop[j] != 0
                o = env.problem.orders[j]
                if m ∈ o.eligmachines[o.ops[env.siminfo.pointercurrentop[j]]]
                    if env.state.executable[j]
                        if parse(Int64, o.id[5:end]) < IDS[o.type]
                            IDS[o.type] = parse(Int64, o.id[5:end])
                        end
                    else
                        if parse(Int64, o.id[5:end]) < IDW[o.type]
                            IDW[o.type] = parse(Int64, o.id[5:end])
                        end
                    end
                end
            end
        ...
        for prod ∈ keys(env.problem.mapindexprods)
            env.siminfo.id_type_machine[prod][m]["scheduling"] = IDS[prod]
            env.siminfo.id_type_machine[prod][m]["waiting"] = IDW[prod]
        end
    end
end
