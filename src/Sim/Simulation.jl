...
function risksetupaction!(env::Env, action::Tuple{Int,Int,String},rng::AbstractRNG)
    ...
    if env.deterministic
        env.siminfo.nextevent[machine][1] = (env.siminfo.time + env.problem.setupstatejob[machine][setup][job]["mean"])
    elseif sample !== nothing
    ...
    env.siminfo.activitytimes[machine] = [[env.siminfo.time + env.problem.setupstatejob[machine][setup][job]["min"], 0.0, 0.0], # store min time
                                            ...
                                            env.deterministic ? ["Deterministic", "None"] : [env.problem.setupstatejob[machine][setup][job]["type"], "None"],
...
function scheduleaction!(env::Env, action::Tuple{Int,Int, String},rng::AbstractRNG)
    ...
    if env.problem.setupstatejob[machine][setup][job]["mean"] == 0
        if env.deterministic
            env.siminfo.nextevent[machine][1] = (env.siminfo.time + env.problem.orders[job].processingtimes[operation][machine]["mean"])
        elseif sample !== nothing
            env.siminfo.nextevent[machine][1] = (env.siminfo.time
        ...
        # schedule and setup
        if env.deterministic
            tmpS = env.siminfo.time + env.problem.setupstatejob[machine][setup][job]["mean"]
            env.siminfo.nextevent[machine][1] = (tmpS + env.problem.orders[job].processingtimes[operation][machine]["mean"])
        elseif sample !== nothing
        ...
        env.siminfo.activitytimes[machine] =    [
                                                    ...
                                                    env.deterministic ? ["Deterministic", "Deterministic"] : [env.problem.setupstatejob[machine][setup][job]["type"], env.problem.orders[job].processingtimes[operation][machine]["type"]],
    ...
    elseif "ois" ∈ env.actionmasks
        id = parse(Int64, env.problem.orders[job].id[5:end])
        orderChange = env.problem.orders[job]
        for m ∈ orderChange.eligmachines[operation]
            OIS = Inf
            OIW = Inf
                ...
                if jobs_old != job
                    ...
                    if env.siminfo.pointercurrentop[jobs_old] != 0
                        if m ∈ ord_old.eligmachines[env.siminfo.pointercurrentop[jobs_old]]
                            if orderChange.type == ord_old.type
                                if env.state.executable[jobs_old]
                                    if parse(Int64, ord_old.id[5:end]) < OIS
                                        OIS = parse(Int64, ord_old.id[5:end])
                                    end
                                else
                                    if parse(Int64, ord_old.id[5:end]) < OIW
                                        OIW = parse(Int64, ord_old.id[5:end])
                                    end
                                end
                            end
                        end
            ...
            if OIS > id
                append!(changesS, m)
                env.siminfo.id_type_machine[orderChange.type][m]["schedule"] = OIS
            end 
            if OIW > id
                append!(changesW, m)
                env.siminfo.id_type_machine[orderChange.type][m]["waiting"] = OIW
            end
        end
    end
    ...
    elseif "ois" ∈ env.actionmasks && env.actionspace != "AIM"
        new_ops = env.siminfo.pointercurrentop[job]
        if new_ops != 0
            id = parse(Int64, env.problem.orders[job].id[5:end])
            orderChange = env.problem.orders[job]
            for m ∈ orderChange.eligmachines[new_ops]
                    ...
                    if jobs_new != job
                        ...
                        if env.siminfo.pointercurrentop[jobs_new] != 0
                            if m ∈ ord_new.eligmachines[env.siminfo.pointercurrentop[jobs_new]]
                                if orderChange.type == ord_new.type
                                    if !env.state.executable[jobs_new]
                                        if parse(Int64, ord_new.id[5:end]) < OIW
                                            OIW = parse(Int64, ord_new.id[5:end])
                                        end
                                    end
                                end
                            end
                ...
                if OIW < id
                    append!(changesW, m)
                    env.siminfo.id_type_machine[orderChange.type][m]["waiting"] = id
                end
            end
        end
    end
    updatestate!(env, action)
    updatedactionmask!(env, action)
    if (!isempty(changesS) || !isempty(changesW))
        if "dds" ∈ env.actionmasks
            updateactionmask_edd_change(env, action, changesS, changesW)
        elseif "ois" ∈ env.actionmasks
            updateactionmask_id_change(env, action, changesS, changesW)
        end
    end
end
        ...
        while(!decisionneeded)# find next event
            ...
            if tmpNext[1] == Inf
                ...
                if all(env.siminfo.nextevent[i][2] == 1 for i ∈ eachindex(env.siminfo.nextevent))
                    ...
                    println("unfinished jobs operations: ", state_of_unfinished_jobs)
                    print(env.siminfo.id_type_machine)
...
function updatestate!(env::Env, event::Tuple{Float64,Int})
    ...
    if "ois" ∈ env.actionmasks update_id!(env) end
