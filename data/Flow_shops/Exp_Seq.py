        ...
        for b in ops:
            for c in ELIGIBLE_MACHINES[a][b]:
                ...
                if data["initial_setup"][c][tmpType]["type"] == "Nonexisting":
                    ...
                    if setups_coupled:
                        SETUPS[c][tmpIndex][0] = round(data["initial_setup"][c][tmpType][exp_measure] / precision)
                    else:
                        SETUPS[c][tmpIndex, 0] = round(data["initial_setup"][c][tmpType][exp_measure] / precision)
...
def expected_sequence(instancetype, objective, filename=None, exp_measure="mean", setups_coupled=True, **kwargs):
    """
    ...
    if instancetype == "doerr":
        basedir = str(pathlib.Path(__file__).parent.resolve()) + "/Benchmark_Doerr/Instances/"
    elif instancetype == "usecase":
        basedir = str(pathlib.Path(__file__).parent.resolve()) + "/Use_Case_Cereals/"
    ...
    for filestr in files:
        ...
        if instancetype == "doerr":
            solutiondir = solutiondir[:-10]
            filename = "seq_" + exp_measure + "_" + filestr[:-5] + ".json"
        else:
            filename = "seq_" + filestr[:-11] + ".json"

        if setups_coupled:
            setup_folder = "/coupled_setups/"
        else:
            setup_folder = "/decoupled_setups/"
        solutiondir = solutiondir + "tailored/" + filestr[:-9] + "/" + obj + setup_folder
...
if __name__ == "__main__":
    expected_sequence("doerr", [1, 0], filename="dpg_def_e80.json", exp_measure="median")
